    ...
    <Section id="reviews" ariaLabelledBy="reviews-heading">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        >
          ...
          <div className="relative w-full h-[330px] overflow-hidden rounded-2xl bg-card text-card-foreground border border-border/80 shadow-md p-5 sm:p-6 flex flex-col justify-between text-center mx-auto transition-all duration-500">
            {/* Header Info Area (Compact & Sleek) */}
            <div className="flex flex-col items-center gap-1.5 pb-2.5 border-b border-border/40 shrink-0">
              <div className="flex items-center gap-3">
                {/* Compact Avatar with Google Badge */}
                <div className="relative">
                  <div
                    className="flex size-10 items-center justify-center rounded-full text-white font-bold text-sm shadow-inner"
                    style={{ backgroundColor: review.avatarBg }}
                  >
                    {review.avatarLetter}
                  </div>
                  <div
                    className="absolute -bottom-0.5 -right-0.5 flex size-4 items-center justify-center rounded-full bg-white shadow-sm border border-slate-200"
                ...
                <div className="text-left">
                  <h3 className="text-primary font-bold text-sm sm:text-base leading-tight">
                    {review.name}{" "}
                    {review.role && (
                      <span className="font-normal text-muted-foreground text-xs sm:inline">
                        • {review.role}
                      </span>
                    )}
                  </h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-muted-foreground text-[11px]">{review.timeAgo}</span>
                    <span className="text-border">•</span>
                    <div className="flex items-center gap-1 text-[10px] font-bold text-[#B5894B] uppercase">
                      <span>VERIFIED REVIEW</span>
                      <CheckCircle2 className="size-3 fill-[#B5894B] text-white" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Testimonial Quote - Larger & More Prominent Text Display */}
            <div className="flex-1 flex items-center justify-center py-2 px-2">
              <p
                className={`text-foreground/90 font-medium text-sm sm:text-base leading-relaxed max-w-xl ${
                  review.isArabic ? "font-serif text-right text-base sm:text-lg" : "text-center"
                }`}
              >
                "{review.quote}"
              </p>
